...
const IMGBB_API_KEY = "58724c8509a2aa71b59f73716b84db65";
const MAX_FILE_SIZE_KB = 80;
...
export default function ProductModal({ isOpen, onClose, shopId, editingProduct }: ProductModalProps) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (kbSize > MAX_FILE_SIZE_KB) {
        return toast.error(`Image still too large (${kbSize.toFixed(1)}KB).`, { id: uploadToast });
      }
  ...
  return (
    // CHANGED: Fixed center alignment on mobile to pull the popup significantly higher.
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-white w-full max-w-md sm:max-w-lg rounded-[28px] shadow-2xl animate-in zoom-in-95 max-h-[85dvh] flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-5 overflow-y-auto custom-scrollbar flex-1 bg-white">
          <div className="space-y-4">
            ...
            <div>
              ...
              <input type="text" required value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full border border-gray-200 bg-gray-50 rounded-xl p-3.5 text-sm font-bold outline-none focus:bg-white focus:ring-2 focus:ring-baltic-blue transition-all" />
            </div>
            
            <div className="grid grid-cols-2 gap-3 items-end">
              <div>
                <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">Cost Price</label>
                <input type="number" required min="0" step="0.01" value={formData.costPrice} onChange={(e) => setFormData({...formData, costPrice: e.target.value})} className="w-full border border-gray-200 bg-gray-50 rounded-xl p-3.5 text-sm font-bold font-ibm outline-none focus:bg-white focus:ring-2 focus:ring-baltic-blue transition-all" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">Selling Price</label>
                <input type="number" required min="0" step="0.01" value={formData.sellingPrice} onChange={(e) => setFormData({...formData, sellingPrice: e.target.value})} className="w-full border border-gray-200 bg-gray-50 rounded-xl p-3.5 text-sm font-bold font-ibm outline-none focus:bg-white focus:ring-2 focus:ring-baltic-blue transition-all" />
              </div>
            </div>

            <div className="grid grid-cols-2 gap-3 items-end">
              <div>
                {/* CHANGED: Shrunk input label to Total Quantity */}
                <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">Total Quantity</label>
                <input type="number" required min="0" value={formData.currentStock} onChange={(e) => setFormData({...formData, currentStock: e.target.value})} className="w-full border border-gray-200 bg-gray-50 rounded-xl p-3.5 text-sm font-bold font-ibm outline-none focus:bg-white focus:ring-2 focus:ring-baltic-blue transition-all" />
              </div>
              {!isNaN(margin) && formData.sellingPrice !== "" && formData.costPrice !== "" ? (
                <div className={`rounded-xl p-3.5 flex justify-between items-center border ${margin >= 0 ? 'bg-green-50 border-green-100 text-sage-green' : 'bg-red-50 border-red-100 text-red-600'}`}>
            ...
            <div className="bg-orange-50/30 border border-orange-100 rounded-xl p-3 mt-2">
               ...
               <div className="flex gap-3">
                <input type="text" placeholder="Batch No" value={formData.batchNo} onChange={(e) => setFormData({...formData, batchNo: e.target.value})} className="w-full border border-gray-200 bg-white rounded-lg p-3 text-xs outline-none focus:ring-2 focus:ring-baltic-blue font-bold shadow-sm" />
                <input type="date" value={formData.expiryDate} onChange={(e) => setFormData({...formData, expiryDate: e.target.value})} className="w-full border border-gray-200 bg-white rounded-lg p-3 text-xs outline-none focus:ring-2 focus:ring-baltic-blue font-bold shadow-sm" />
              </div>
            </div>
        ...
        <div className="p-4 sm:p-6 shrink-0 border-t border-gray-100 bg-gray-50">
          <button onClick={handleSubmit} disabled={isSubmitting} className="w-full bg-baltic-blue text-white font-bold py-4 rounded-xl shadow-lg hover:bg-rich-cerulean active:scale-[0.98] transition-all disabled:opacity-70 flex justify-center items-center gap-2">
